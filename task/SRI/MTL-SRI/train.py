...
def get_vocab(args):
    ...
    w2i_attr, i2w_attr = load_vocabulary(
        os.path.join(args.data_dir, "vocab_attr.txt"))  # 实体归一化 [咳嗽 咳嗽 null null null null]
    w2i_type, i2w_type = load_vocabulary(
        os.path.join(args.data_dir, "vocab_type.txt"))  # 实体属性 [1 1 null null null null null]
    vocab_dict = {
        "w2i_char": w2i_char,
        "i2w_char": i2w_char,
        "w2i_bio": w2i_bio,
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        while epoches < args.num_epoch:
            ...
            if batches == 0:
                ...
                logger.info(
                    "output_seq_attr: " + " ".join([vocab_dict['i2w_attr'][i] for i in outputs_seq_attr_batch[0]]))
                logger.info(
                    "output_seq_type: " + " ".join([vocab_dict['i2w_type'][i] for i in outputs_seq_type_batch[0]]))
                logger.info("###############################")
            ...
            if batches % 100 == 0:
                ...
                (preds_kvpair, golds_kvpair) = evaluate(sess, model, data_processor_valid, vocab_dict, max_batches=100,
                                                        batch_size=1024)
...
def predict(args, model, data_processor_test, vocab_dict):
    ...
    with tf.Session() as sess:
        ...
        (chars_seq, preds_kvpair, eids) = predict_evaluate(sess, model, data_processor_test, vocab_dict,
...
def main():
    ...
    parser.add_argument('--test_output_file', '-tof', type=str, default='submission_track1_task2.json',
    ...
    parser.add_argument('--batch_size', '-bs', type=int, default=128, help='Batch size for trainging')
