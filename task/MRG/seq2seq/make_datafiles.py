...
def get_tokenized_data(medi_tokenized_dir, df_id_report):
    ...
    for i in range(len(df_id_report)):
        ...
        if split == 'train':
            for j in range(2):
                with open(os.path.join(medi_tokenized_dir, split + '_' + str(id_) + '_' + str(j)), 'w',
                          encoding='utf-8') as f:
...
def deal_with_json():
    ...
    for eid in eids_test:
        ...
        split = 'test'
        id_report.append([eid, self_report, dialogue, report1, report2, split])
    df_id_report = pd.DataFrame(id_report,
                                columns=['example_id', 'self_report', 'dialogue', 'report1', 'report2', 'split'])
